...
def read_layout_from_sheet(
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        layer = ws.cell(r, idx("layer") or 0).value if idx("layer") else None
        ...
        if t == "wall":
            ...
            if None not in (x1, y1, x2, y2):
                walls.append(Wall(x1=x1, y1=y1, x2=x2, y2=y2, layer=layer))
            ...
            inputs.append(Input(id=iid, x=float(x), y=float(y), layer=layer))
            ...
            outputs.append(Output(id=oid, x=float(x), y=float(y), input=float(input_machine), layer=layer))
            ...
            cols.append(Column(x=float(x), y=float(y), w=float(w), d=float(d), rot=float(rot), layer=layer))
            ...
            side_in = _s(ws.cell(r, idx("sidein") or 0).value) if idx("sidein") else "Left"
            offset_in = _f(ws.cell(r, idx("offsetin") or 0).value) if idx("offsetin") else None
            side_out = _s(ws.cell(r, idx("sideout") or 0).value) if idx("sideout") else "Right"
            offset_out = _f(ws.cell(r, idx("offsetout") or 0).value) if idx("offsetout") else None
    ...
    if machines:
        ...
        for i, m in enumerate(machines):
            ...
            for up in parts:
                if input_id and up == input_id:
                    edges.append((None, i))
                    print(f"Verbindung {input_id} -> {m.id} (entry)")
                    continue

                if up in id_to_idx:
                    edges.append((id_to_idx[up], i))
                    print(f"Verbindung {up} -> {m.id}")
                    continue
        ...
        if output_obj is not None:
            ...
            if out_target is not None:
                ...
                print(f"Verbindung {machines[out_target].id} -> {output_obj.id} (exit)")

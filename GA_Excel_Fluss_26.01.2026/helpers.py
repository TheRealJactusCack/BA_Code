...
def machine_water_point(m: Dict) -> Tuple[float, float]:
    """Weltkoordinate des Wasseranschlusses einer Maschine."""
    idx = int(m.get("idx", 0))
    w_m, d_m = config.MACHINE_SIZES[idx]
    water = getattr(config, "MACHINE_WATER", [])
    wd = water[idx] if idx < len(water) else {}

    side = str(wd.get("side_water", None)).strip().lower()
    default_off = (float(d_m) / 2.0) if side in {"left", "right"} else (float(w_m) / 2.0)
    offset = float(wd.get("offset_water", default_off))
...
def machine_worker_point(m: Dict) -> Tuple[float, float]:
    ...
    side = str(wd.get("side_worker", None)).strip().lower()
    default_off = (float(d_m) / 2.0) if side in {"left", "right"} else (float(w_m) / 2.0)
...
def distance_cost(ind: List[Dict], config: any) -> float:
    ...
    for e in routed["worker"]:
        length = float(e["length_m"])
        cost += no_path_penalty if not math.isfinite(length) else length #quadratische kosten damit kurze wege bevorzugt werden

...
def _walls_to_polygon(walls_xy: List[Tuple[Tuple[float, float], Tuple[float, float]]]) -> Optional[List[Tuple[float, float]]]:
    """
    Rekonstruiert ein geschlossenes Polygon aus Wall-Segmenten.
...
def read_layout_from_sheet(
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        if t == "wall":
            ...
            worker = int(_f(ws.cell(r, idx("worker") or 0).value) or 1)
            side_w = _s(ws.cell(r, idx("side_worker") or 0).value) if idx("side_worker") else "Top"
            raw_offset_w = _f(ws.cell(r, idx("offset_worker") or 0).value) if idx("offset_worker") else None
            # Offset ist entlang der Kante gemessen:
            # left/right -> entlang d, top/bottom -> entlang w
            if raw_offset_w is None:
            ...
            #print(f"Machine {mid} mit Arbeiter {worker} an Seite {side_w} mit Offset {offset_w}")
            ...
            machines.append(
                MachineRow(
                    ...
                    worker = worker,
                    side_worker = side_w,
                    offset_worker = float(offset_w),
                )
            )

    return walls, inputs, outputs, cols, machines, meta
...
def apply_excel_layout_to_config(xlsx_path: str, sheet_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    walls, inputs, outputs, cols, machines, meta = read_layout_from_sheet(xlsx_path, sheet)
    ...
    if machines:
        ...
        config.MACHINE_WORKERS = []
        for k in machines:
            config.MACHINE_WORKERS.append(
                {
                    "worker": int(k.worker),
                    "side_worker": str(k.side_worker).strip(),
                    "offset_worker": float(k.offset_worker),
                }
            )

        config.MACHINE_PORTS = []
        for m in machines:
            config.MACHINE_PORTS.append(
                {
        ...
        # WORKER_CONNECTIONS befüllen
        w_edges: List[Tuple[Optional[int], Optional[int]]] = []
        for i, w in enumerate(machines):
            for j, m in enumerate(machines):
                if j <= i:
                    continue
                if w.worker == m.worker:
                    idx_a = id_to_idx[w.id]
                    idx_b = id_to_idx[m.id]
                    w_edges.append((idx_a, idx_b))
                    #print(f"Worker-Verbindung {w.id} <-> {m.id} (Arbeiter {w.worker})")

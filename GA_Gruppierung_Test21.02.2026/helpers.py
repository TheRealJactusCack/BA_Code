...
def occupied_cells(m: Dict, Clearance) -> set[Tuple[int, int]]:
    ...
    if Clearance:
        if 0 <= idx < len(config.MACHINE_WORKERS) and config.MACHINE_WORKERS[idx] is not None:
            x_top_left, y_top_left = get_worker_clearance(m)
            n = int(round(1 / config.GRID_SIZE))
            for dwx in range(n):
                for dwy in range(n):
                    col = x_top_left + dwx
                    row = y_top_left + dwy
...
def compute_routed_edges(ind: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    for a, b in w_edges:
        ...
        start_x_top_left, start_y_top_left = get_worker_clearance(ind[a_idx])
        end_x_top_left, end_y_top_left = get_worker_clearance(ind[b_idx])
        StartGrid: set[Tuple[int, int]] = set()
        EndGrid: set[Tuple[int, int]] = set()
        Meter = int(round(1 / config.GRID_SIZE))

        if start_x_top_left is None or start_y_top_left is None:
            continue
        if end_x_top_left is None or end_y_top_left is None:
            continue

        for dwx in range(Meter):
            for dwy in range(Meter):
                StartCol = start_x_top_left + dwx
                StartRow = start_y_top_left + dwy
                EndCol = end_x_top_left + dwx
                EndRow = end_y_top_left + dwy
                if 0 <= StartCol < config.GRID_COLS and 0 <= StartRow < config.GRID_ROWS:
                    StartGrid.add((StartCol, StartRow))
                if 0 <= EndCol < config.GRID_COLS and 0 <= EndRow < config.GRID_ROWS:
                    EndGrid.add((EndCol, EndRow))
...
Edge = Tuple[Optional[int], Optional[int], float]
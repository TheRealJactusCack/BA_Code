...
def match_weights(raw: str, n: int, *, ctx: str) -> List[float]:
     s = str(raw or "").strip()
     if not s:
         return [1.0] * int(n)
     ...
     if len(toks) != int(n):
         raise ValueError(
             f"Gewicht-Anzahl passt nicht zu Input-Anzahl bei {ctx}: "
             f"inputs={n} gewichte={len(toks)} raw='{s}'"
         )
     ...
     for t in toks:
         try:
             ...
             raise ValueError(f"Ungültiges Gewicht '{t}' bei {ctx}") from e
     return out
...
def read_layout_from_sheet(
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        if t == "wall":
            ...
            x = _f(ws.cell(r, idx("x") or 0).value)
            y = _f(ws.cell(r, idx("y") or 0).value)
            input_machine = _s(ws.cell(r, idx("input") or 0).value) if idx("input") else ""
            gewicht_raw = _s(ws.cell(r, idx("weight") or 0).value) if idx("weight") else ""
            if x is None or y is None:
            ...
            MaschineLabel = _s(ws.cell(r, idx("label") or "Fehler1").value) if idx("label") else "Fehler2"
            ...
            if w_x1 is None or w_y1 is None or w_x2 is None or w_y2 is None:
                continue
            ...
            if g_x1 is None or g_y1 is None or g_x2 is None or g_y2 is None:
                continue
            ...
            if o_x1 is None or o_y1 is None or o_x2 is None or o_y2 is None:
                continue
    ...
    if machines:
        ...
        for i, m in enumerate(machines):
            ...
            if m.x is not None and m.y is not None:
                FixedX, FixedY = to_internal_xy(float(m.x), float(m.y))
        ...
        for i, m in enumerate(machines):
            ...
            raw_gew = str(m.gewicht or "").strip() #=========================================================================================gewicht=================================
            if not raw:
                continue
            parts = [p.strip() for p in raw.replace(";", ",").split(",") if p.strip()]
            weights = match_weights(raw_gew, len(parts), ctx=f"Maschine {m.id}")
            for up, w in zip(parts, weights):
                if input_id and up == input_id:
                    edges.append((None, i, float(w)))
                    continue

                if up in id_to_idx:
                    edges.append((id_to_idx[up], i, float(w)))
                    continue

                raise ValueError(f"input referenziert unbekannte Maschine/Quelle {up} bei {m.id}")

        #2) (Maschine->Senke) explizit aus Output.input speichern
        if output_obj is not None:
            raw_out = str(output_obj.input or "").strip()
            raw_out_gew = str(output_obj.gewicht or "").strip()

            parts_out = [o.strip() for o in raw_out.split(";") if o.strip()]
            o_weights = match_weights(raw_out_gew, len(parts_out), ctx=f"Maschine {m.id}")

            for up, w in zip(parts_out, o_weights):
                if up in id_to_idx:
                    edges.append((id_to_idx[up], None, float(w)))
                    continue
                else:
        ...
        mm_edges: List[Tuple[int, int]] = [(int(a), int(b)) for (a, b, _w) in edges if a is not None and b is not None]

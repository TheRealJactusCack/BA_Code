        ...
        if 0 <= idx < len(config.MACHINE_WORKERS) and config.MACHINE_WORKERS[idx] is not None:
            ...
            for dwx in range(n):
                for dwy in range(n):
                    c = wx + dwx
                    r = wy + dwy
                    if 0 <= c < config.GRID_COLS and 0 <= r < config.GRID_ROWS:
                        out.add((c, r))
...
def _astar_worker_path(start: Tuple[int, int], goal: Tuple[int, int], blocked: set[Tuple[int, int]]) -> Optional[List[Tuple[int, int]]]:
    if start == goal:
        return [start]
    #Start/Ziel dürfen in "blocked" liegen (da Ports häufig in der Maschine liegen)
    blocked_local = set(blocked)
    blocked_local.discard(start)
    blocked_local.discard(goal)

    def h(c: Tuple[int, int]) -> int: #Astar heuristik (Abstand aktueller Node zum Ziel)
        return abs(c[0] - goal[0]) + abs(c[1] - goal[1])
    """// A* (star) Pathfinding

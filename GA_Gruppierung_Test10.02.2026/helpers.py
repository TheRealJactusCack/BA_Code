...
def distance_cost(ind: List[Dict], config: any) -> float:
    ...
    for m in routed["material"]:
        if m["a"] is None:
            ...
            length = float(m["length_m"])
            cost += no_path_penalty if not math.isfinite(length) else length * 2  # Materialflüsse höher gewichtet
    ...
    for w in routed["water"]:
        ...
        cost += no_path_penalty if not math.isfinite(length) else length * 0.5

...
def _print_progress(done: int, total: int, started_at: float) -> None:
    ...
    msg = f"{bar} {done}/{total} ({pct:5.1f}%) | elapsed {_fmt_hhmmss(elapsed)} | ETA {_fmt_hhmmss(eta)} | Avg. time per run {per_item:.2f}"
...
def main() -> int:
    ...
    params: List[DOEParam] = [
        DOEParam(key = "POPULATION_SIZE", start = 20, stop = 50, steps = 2, kind = "int"),
        DOEParam(key = "GENERATIONS", start = 5, stop = 20, steps = 2, kind = "int"),
        DOEParam(key = "SWAP_PROP", start = 0, stop = 0.2, steps = 2, kind = "float"),
        DOEParam(key = "MUTATION_PROB", start = 0, stop = 0.2, steps = 2, kind = "float"),
        DOEParam(key = "MUTATION_POS_STD", start = 0, stop = 0.2, steps = 2, kind = "float"),
        DOEParam(key = "MUTATION_ROT_PROB", start = 0, stop = 0.2, steps = 2, kind = "float"),
        DOEParam(key = "ELITE_KEEP", start = 2, stop = 50, steps = 2, kind = "int"),
        DOEParam(key = "GRID_SIZE", start = 0.25, stop = 0.5, steps = 2, kind = "float")
    ]
    ...
    headers = ["Run","Layout", "BestScore", "last Improved gen"] + [p.key for p in params]
    rows: List[List[Any]] = []
    ...
    for run_idx, point in enumerate(points, start=1):
        ...
        best_ind, best_score, last_improved_gen = run_ga(generations, progress_callback=None)

        # Score robust behandeln, falls GA kein Ergebnis liefert
        score = float(best_score) if best_ind is not None else float("inf")

        # Ergebniszeile bauen
        row = [run_idx, sheet_name, score, last_improved_gen] + [point.get(p.key) for p in params]
        rows.append(row)

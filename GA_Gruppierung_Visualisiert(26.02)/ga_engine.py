...
def Create_Groups() -> List[Tuple[int,int]]: 
    Gruppen = [None] * config.MACHINE_COUNT
    workers = getattr(config, "MACHINE_WORKERS", [])
    for Index, Maschine in enumerate (workers): 
        if Maschine is None:
            continue
        if Gruppen[int(Maschine["worker"]) - 1] is None:
            Gruppen[int(Maschine["worker"]) - 1] = (Index + 1, None)
        else:
            Gruppen[int(Maschine["worker"]) - 1][1] = Index + 1
    return Gruppen
...
def _group_machines(best_ind: List[Dict]) -> List[List[Dict]]:
    ...
    #Wass brauche ich:
    # 1. Eine Liste der MaschinenArbieter (Annahme, 2 Maschinen pro Arbeiter, Numeriert 1 ... n) 
    # 2. Die Liste muss durchgegangen werden
    # 3. maschinen mit dem gleichen Arbeitern in eine Gruppe
    # 1 - 3 macht die Fuktion Create_Groups
    ...
    for Index in MachineGroups:
        if MachineGroups[Index] is None:
            continue
        MaschineFixed = MachineGroups[Index][0]
        MaschineTest = MachineGroups[Index][1]
    ...
    for i, (a, b) in enumerate(zip(best_ind[::2], best_ind[1::2])):
        #Kostenberechung zwischen Maschinen mit dem gleichen Worker (Proof of concept)
        ...
        while True:
            ...
            #Brutal schlechtr Code btw hier drüber
...
def run_ga(generations: int, progress_callback=None) -> Tuple[Optional[List[Dict]], float]:
    ...
    """
    BestIndCopy = copy.deepcopy(best_ind)
    GroupsForGA = Optimize_Groups(BestIndCopy)
    # GroupsForGA = [{'leader': 0, 'members': [0, 1], 'local': {1: {'MemberX': 24, 'MemberY': 0, 'MemberZ': 0}}}, {'leader': 2, 'members': [2, 3], 'local': {3: {'MemberX': 24, 'MemberY': 6, 'MemberZ': 0}}}, {'leader': 4, 'members': [4, 5], 'local': {5: {'MemberX': -24, 'MemberY': 0, 'MemberZ': 0}}}]

...
def run_ga(generations: int, progress_callback=None) -> Tuple[Optional[List[Dict]], float]:
    ...
    if config.CREATE_GROUPS:
        ...
        best_ind = None
        best_score = float("inf")
        IndSwaps = [False for _ in group_pop]

        for g in range(1, int(generations) + 1):
            if config.STOP_REQUESTED:
                if progress_callback:
                    try:
                        progress_callback(g, generations, best_score, best_ind, group_pop, False)
                    except TypeError:
                        progress_callback(g, generations, best_score, best_ind)
                break

            scores = [fitness(ind) for ind in group_pop]
            paired = list(zip(scores, group_pop, IndSwaps))
            paired.sort(key=lambda p: p[0])
            ...
            if progress_callback:
                progress_callback(g, generations, best_score, best_ind, group_pop, SwapImproved)

...
class MainWindow(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Fabrikplaner")
        ...
        self.sheet_combo.setFixedSize(200, 25)
        ...
        self.SwapTookPlace: list[bool] = []

        self.curve = self.graph_widget.plot(self.fitness_Point_X, self.fitness_Point_Y, symbol="d", symbolSize = 5, pen=pyqtgraph.mkPen("#2D7FF9"), brush = pyqtgraph.mkBrush("#2D7FF9"))
        self.curve.setZValue(10)

        self.SwapMarker = pyqtgraph.ScatterPlotItem(x = [], y = [], symbol="d",symbolSize = 5, pen = pyqtgraph.mkPen("#FF0000"), brush = pyqtgraph.mkBrush("#FF0000"))
        self.SwapMarker.setZValue(20)
        self.graph_widget.addItem(self.SwapMarker)
        self.SwapX: list[int] = []
        self.SwapY: list[float] = []
        self.SwapsHappened: set[int] = set()
    ...
    def start_ga(self):
        ...
        self.SwapTookPlace.clear()
        self.curve.setData(self.fitness_Point_X, self.fitness_Point_Y)

        self.SwapX.clear()
        self.SwapY.clear()
        self.SwapsHappened.clear()
        self.SwapMarker.setData(x = [], y = [])
        ...
        def callback(g, total, best_score_cb, best_ind_cb, population=None, Swap = False):
            ...
            if Swap:
                ...
                if GenIndex not in self.SwapsHappened:
                    self.SwapsHappened.add(GenIndex)

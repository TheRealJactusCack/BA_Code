...
def route_world_to_world_cells(
    ...
    *,
    ...
    path_cells ist der A*-Zellenpfad (4er Nachbarschaft). Damit können wir z.B.
    Worker-Wege als flächige Clearance (Breite) markieren.
    """
    s_cell = _cell_of_world(*start_world)
    e_cell = _cell_of_world(*end_world)
...
def compute_routed_edges(ind: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    for a, b in w_edges:
        ...
        pts, length_m, cells = route_world_to_world_cells(p1, p2, blocked=blocked, blocked_sig=blocked_sig)

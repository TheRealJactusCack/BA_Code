...
def _clamp_cell(col, row):
    c = max(0, min(config.GRID_COLS - 1, int(col)))
    r = max(0, min(config.GRID_ROWS - 1, int(row)))
    return c, r


def get_terminal_cell(m, kind='out'):
    """Gibt die Zell-Koordinate (col,row) zurück, die als Start/Endpunkt für Materialfluss dient.
    Versucht zuerst den exakten Output/Input-Punkt und springt bei Bedarf ein Feld in Richtung der Maschine-Ausrichtung."""
    if kind == 'out':
        px, py = machine_output_point(m)
    else:
        px, py = machine_input_point(m)
    col = int(px // config.GRID_SIZE)
    row = int(py // config.GRID_SIZE)
    col, row = _clamp_cell(col, row)
    # if this cell is inside the machine footprint, step away along direction
    occ = set(occupied_cells(m))
    if (col, row) in occ:
        dx = px - m['x']
        dy = py - m['y']
        step_c = 1 if dx > 0.0 else (-1 if dx < 0.0 else 0)
        step_r = 1 if dy > 0.0 else (-1 if dy < 0.0 else 0)
        if step_c == 0 and step_r == 0:
            # fallback: try to the right
            step_c = 1
        cand = (col + step_c, row + step_r)
        cand = _clamp_cell(*cand)
        return cand
    return col, row


def find_manhattan_path(start, goal, blocked_cells=None):
    """BFS auf Raster (4-neighbor) von start (col,row) zu goal. blocked_cells ist eine Menge von Zellen, die nicht betreten werden dürfen.
    Gibt Liste von (col,row) vom start bis goal zurück oder None wenn kein Pfad gefunden.
    """
    ...
    while q:
        ...
        for dc, dr in neigh:
            ...
            if nxt in came:
                continue
            if nxt in blocked_cells and nxt != g:
                continue
            came[nxt] = cur
            if nxt == g:
                # reconstruct
...
def cells_to_world(path_cells):
    """Konvertiert Liste von (col,row) Zellen zu Welt-Koordinaten (Zentren)."""
...
def random_machine_nonoverlap(idx, occupied_set, max_attempts=200):
    ...
    # Rotation: wenn aus Excel fix vorgegeben -> verwenden, sonst zufällig
    if hasattr(config, 'MACHINE_ROTATIONS') and isinstance(getattr(config, 'MACHINE_ROTATIONS'), (list, tuple)) and idx < len(config.MACHINE_ROTATIONS):
        z = int(config.MACHINE_ROTATIONS[idx])
    else:

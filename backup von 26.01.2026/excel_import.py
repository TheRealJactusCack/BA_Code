...
def read_layout_from_sheet(
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        if t == "wall":
            ...
            side_w = (_s(ws.cell(r, c).value) if (c := idx("side_worker")) and ws.cell(r, c).value not in (None, "") else None)
            raw_offset_w = (_f(ws.cell(r, c).value) if (c := idx("offset_worker")) and ws.cell(r, c).value not in (None, "") else None)

            print(f"Machine {mid} mit Arbeiter {worker} an Seite {side_w} mit Offset {raw_offset_w}")
...
def apply_excel_layout_to_config(xlsx_path: str, sheet_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    if machines:
        ...
        # Für 2 Maschinen entsteht automatisch A->B und B->A
